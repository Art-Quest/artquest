...
const Game = ({ setUiMode }) => {
  ...
  const gameScale = window.innerWidth > 960 ? 0.5 : 1;
  ...
  const playerPostRealX =
    vwUnit *
    (calcXPos(playerPos.x, playerPos.y, false, 0, 0, gameScale) +
      20 * gameScale);
  const playerPostRealY =
    vwUnit *
    (calcYPos(playerPos.x, playerPos.y, false, 0, 0, gameScale) +
      10 * gameScale);
  ...
  const getItem = (x, y, z) => {
    ...
    const foundId = ids.find(
      (id) => itemsState[z][id].x === x && itemsState[z][id].y === y
    );
    if (foundId) {
  ...
  const changePlayerPosition = (dx, dy, dz) => {
    ...
    if (validMove) {
      ...
      if (
        itemOnSpot &&
        objTypes.itemTypes[itemOnSpot.objType].action === "talk"
      ) {
        setDialogueOpenNpc(itemOnSpot.objType);
      }
      if (
        itemOnSpot &&
        objTypes.itemTypes[itemOnSpot.objType].action === "sign"
      ) {
      ...
      if (
        itemOnSpot &&
        objTypes.itemTypes[itemOnSpot.objType].action === "marker"
      ) {
        const newSpot = {
        ...
        setTeleportSpots([...teleportSpots, newSpot]);
  ...
  if (
    itemOnSpot &&
    objTypes.itemTypes[itemOnSpot.objType].action === "teleport"
  ) {
  ...
  let availableActions = dirs.map((dir) => {
    ...
    const validByItems =
      !nextItem || (nextItem && objTypes.itemTypes[nextItem.objType].passable);

    if (tmpSpot) {
      if (
        validByItems &&
        (dir.lbl === "n" || dir.lbl === "s") &&
        tmpSpot.walls.n.objType === "none" &&
        objTypes.tileTypes[tmpSpot.objType].walkable
      ) {
        return dir.lbl;
      }
      if (
      ...
      switch (invAction.actionType) {
        case "fill-bowl": {
          ...
          if (bowl) {
            ...
            if (numLeft > 0) {
              toast(
                `Put the noodles in the bowl. Nearly lunch time. Just ${numLeft} more to go!`
              );
            } else {
        ...
        case "door": {
          ...
          const currentSpotS = getSpot(
            playerPos.x,
            playerPos.y + 1,
            playerPos.z
          );
          const currentSpotE = getSpot(
            playerPos.x + 1,
            playerPos.y,
            playerPos.z
          );
          ...
          if (currentSpot.walls.n.objType === `door${invAction.special}`) {
            ...
            currentSpot.walls.w.objType === `door${invAction.special}`
          ) {
            changeWallType(
              playerPos.x,
              playerPos.y,
            ...
            currentSpotS &&
            currentSpotS.walls.n.objType === `door${invAction.special}`
          ) {
            changeWallType(
              playerPos.x,
              playerPos.y + 1,
            ...
            currentSpotE &&
            currentSpotE.walls.w.objType === `door${invAction.special}`
          ) {
            changeWallType(
              playerPos.x + 1,
              playerPos.y,
  ...
  const itemIdsLayer0 = itemIds.filter(
    (id) => objTypes.itemTypes[itemsState[playerPos.z][id].objType].layer === 0
  );
  const itemIdsLayer1 = itemIds.filter(
    (id) => objTypes.itemTypes[itemsState[playerPos.z][id].objType].layer === 1
  );

  const display_items0 = React.useMemo(
    () =>
      itemIdsLayer0.map((id, idx) => (
        <Item
          key={idx}
  ...
  const display_items1 = React.useMemo(
    () =>
      itemIdsLayer1.map((id, idx) => (
        <Item
          key={idx}
          item={itemsState[playerPos.z][id]}
          calcXPos={calcXPos}
  ...
  const display_tiles = React.useMemo(
    () =>
      areaState[playerPos.z].map((item, idx) => (
        <Tile
          tileData={item}
          key={idx}
          calcXPos={calcXPos}
  ...
  return (
    <div className={css.game}>
      <div className={css.boardWrap}>
        <div
          className={`${css.board} ${playerAnim && css.anim}`}
          style={{
            left: globalOffsetX,
            top: globalOffsetY,
          }}
        >
          ...
          />
          <div
            className={`${css.fader} ${inTransition !== "" && css.active}`}
            style={{
              zIndex: maxZ - 2,
            }}
          />
      ...
      />
      <Sign text={dialogueOpenSign} handleSignClose={handleSignClose} />
      <TeleporterDialog
      ...
      />
      {numNoodles === maxNoodles && (
        <GameOver setUiMode={setUiMode} resetGame={resetGame} />
      )}
      ...
      {numNoodles === maxNoodles && <GameOverConfetti />}
