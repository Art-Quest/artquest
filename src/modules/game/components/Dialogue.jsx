...
const Dialogue = ({ dialogueOpenNpc, handleDialogClose }) => {
  ...
  const convo =
    dialogueOpenNpc !== "" && cursor !== "exit"
      ? dialogueData[dialogueOpenNpc]
      : {};
  ...
  return (
    <Dialog open={dialogueOpenNpc !== ""} className={css.dialogueModal}>
      ...
      <DialogContent dividers className={css.quoteWrap}>
        <Grid container spacing={0}>
          ...
          <Grid item xs={12} sm={3} className={css.vCenter}>
            <img
              src={`/images/items/${dialogueOpenNpc}.svg`}
              alt={dialogueOpenNpc}
      ...
      <DialogActions className={css.options}>
        <Grid container spacing={2}>
          {convo.paths[cursor].options.map((option, idx) => (
            <Grid item xs={12} key={idx}>
              <Button
                className={css.optionButton}
                variant="outlined"
                onClick={() => handleDialogueClick(option.slug)}
              >
                {option.label}
              </Button>
            </Grid>
