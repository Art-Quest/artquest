    ...
    >
      ...
      <DialogContent dividers className={css.content}>
        <Grid container spacing={3}>
          <Grid item xs={12}>
            Using your nifty teleporter, you can go to any of the markers
            you&apos;ve visited before. Select a spot from the list below.
          </Grid>
        </Grid>
      </DialogContent>
        ...
        <Grid container spacing={1}>
          {teleportSpots.map((spot, idx) => (
            <Grid key={idx} item xs={12} sm={6}>
              <Button
                className={css.teleportOption}
                variant="outlined"
                onClick={() => handleOptionClick(spot)}
              >
